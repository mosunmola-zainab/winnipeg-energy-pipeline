...
- `bronze.ingestion_runs` — one row per ingestion attempt. Confirmed columns: `run_id` (integer identity primary key — see identity-vs-UUID note below), `status` (`running` / `succeeded` / `failed`), `started_at`, `completed_at`, `source_domain`, `source_dataset_id` (the Socrata domain and dataset targeted by that attempt), `rows_fetched`, `rows_loaded`, `last_hydro_gas_id` (the last `hydro_gas_id` read, tracking progress against the deterministic pagination order described below), `observed_fields` (the schema-drift field set — see Schema drift below), and `error_message` (populated on failure). These columns exist in the schema now but are only populated once the corresponding extraction/loading ETL work is implemented — that work is not yet built. A `retry_count` column is deliberately deferred until retry telemetry is implemented.
...
**Why an integer identity `run_id`, not a UUID.** This pipeline has exactly one writer — a single Airflow-scheduled task, never concurrent, never multi-node. UUIDs solve coordination-free ID generation across independent writers, which doesn't apply here. A `BIGINT` identity gives free monotonic ordering, is cheaper to store and index across `utility_billing_raw` (which carries `run_id` on every source row per run), and is far easier to reference in logs and queries than a UUID. This is a deliberate simplicity choice, not an oversight.
...
**Snapshots.** Full snapshots are preserved per `run_id` — the same `hydro_gas_id` recurs once per successful run, by design, not deduplicated across runs. A failed run must never replace the previous successful snapshot; `bronze.current_snapshot` only ever points at a run that completed and passed validation. This is a promotion-time invariant enforced by the future application/loader logic that writes to `current_snapshot`, not by a database trigger or constraint: the foreign key on `current_snapshot.run_id` only guarantees the referenced run exists, not that its `status` is `succeeded`. A database trigger is a possible future defense-in-depth option if a second writer to `current_snapshot` is ever introduced, but is not needed while there is a single writer.
...
**Schema drift.** Each run records the set of source fields actually observed, stored on `bronze.ingestion_runs.observed_fields`. A new non-critical field is recorded and warned about, not treated as a run failure.
